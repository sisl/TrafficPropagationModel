...
export same_lane, same_tile, same_lane_and_tile
export closest_node_to_extind, closest_node_above_extind
export neighbor_north, neighbor_east, neighbor_south, neighbor_west
export neighbor_north, has_neighbor_east, has_neighbor_south, has_neighbor_west, extisting_neighbor_tiles, extisting_neighbor_tiles_inclusive
...
export rndf2streetmap, curves2streetmap
export curves2rndf
...
const SATURATION_DISTANCE_BETWEEN       = 400.0 # [m]
...
type StreetLane
	...
	has_training_node :: Bool # true iff curve includes a destination node leaving this lane
end
type StreetSegment
...
type StreetNetwork
	...
	function StreetNetwork()
		new(Dict{(Int,Int),NetworkTile}(), graph(StreetNode[],Edge{StreetNode}[]))
	end
end
...
type TilePoint2DProjectionResult
	curvept    :: Vector{Float64} # [s,x,y,θ,κ,κ_d]
	extind     :: Float64
	tile       :: NetworkTile
	laneid     :: LaneID
	sqdist     :: Float64
	...
	function TilePoint2DProjectionResult(
		...
		self = new(curvept, extind, tile, laneid, sqdist, successful)
		self
	end
end
...
function lla2xyz( lat::Real, lon::Real, alt::Real )
	...
	const e = sqrt(1.0-(b/a)^2) # eccentricity
...
function tile_bounds(tile::NetworkTile)
	e, n = tile.index_e, tile.index_n
	min_e = tile_lowerbound_east(e)
    max_e = tile_upperbound_east(e)
    min_n = tile_lowerbound_north(n)
    max_n = tile_upperbound_north(n)
...
function has_lane(sn::StreetNetwork, tag::LaneTag)
	if has_tile(sn, tag.index_e, tag.index_n)
		...
		return haskey(tile.segments, id.segment) && has_lane(tile.segments[id.segment], id.lane)
	end
	return false
end
...
has_next_lane(sn::StreetNetwork, lane::StreetLane) = out_degree(lane.nodes[end], sn.graph) > 0
...
has_prev_lane(sn::StreetNetwork, lane::StreetLane) = in_degree(lane.nodes[1], sn.graph) > 0
...
function closest_node_to_extind(lane::StreetLane, extind::Float64)
	# do a linear search
	# TODO(tim): do a bisection search
	best_ind = 0
	best_diff = Inf
...
function closest_node_above_extind(sn::StreetNetwork, lane::StreetLane, extind::Float64)
	# do a linear search
...
function extisting_neighbor_tiles(sn::StreetNetwork, tile::NetworkTile)
	...
	for i = 1 : 8
		...
		if has_tile(sn, tile.index_e + dx, tile.index_n + dy)
			retval[count+=1] = get_tile(sn, tile.index_e + dx, tile.index_n + dy)
		end	
	end

	return retval[1:count]
end
...
function project_point_to_tile(easting::Float64, northing::Float64, tile::NetworkTile )
	...
	for (segment_id, segment) in tile.segments
		for (lane_id, lane) in segment.lanes
			...
			de = pt[Curves.XIND]-easting
			dn = pt[Curves.YIND]-northing
...
function _distance_to_lane_merge(
	...
	dist = node.d_along - curve_at(lane.curve, extind)[SIND]
...
function distance_to_lane_merge(
	...
	if node.extind > extind
        min(node.d_along - curve_at(lane.curve, extind)[SIND] + node.d_merge, max_dist)
    else
        min(curve_at(lane.curve, extind)[SIND] - node.d_along + node.d_merge, max_dist)
    end
end
function distance_to_lane_split(
	...
	node = closest_node_to_extind(lane, extind)
    if node.extind > extind
        min(node.d_along - curve_at(lane.curve, extind)[SIND] + node.d_split, max_dist)
    else
        min(curve_at(lane.curve, extind)[SIND] - node.d_along + node.d_split, max_dist)
    end
end
function distance_to_lane_end(
	...
	node = closest_node_to_extind(lane, extind)
    if node.extind > extind
        min(node.d_along - curve_at(lane.curve, extind)[SIND] + node.d_end, max_dist)
    else
        min(curve_at(lane.curve, extind)[SIND] - node.d_along + node.d_end, max_dist)
    end
end
...
function frenet_distance_between_points(
	...
	if projA.successful
		...
		while !done
			...
			if !isapprox(extind, 1.0) && !isapprox(extind, length(lane.curve))
...
function rndf2streetmap(rndf::RNDF; verbosity::Int=0, convert_ll2utm::Bool=true)
	...
	for segment in values(rndf.segments)
		for lane in values(segment.lanes)
			...
			for i in new_order[2:end]
				nextnode = node_map[WaypointID(segment.id, lane.id.lane, ids[i])]
	...
	for segment in values(rndf.segments)
		for lane in values(segment.lanes)
			for (exit, entry) in lane.exits
				exit_waypoint = WaypointID(segment.id, lane.id.lane, exit)
	...
	for segment in values(rndf.segments)
		for lane in values(segment.lanes)
			...
			while first_node_index ≤ n_pts_in_lane
				...
				final_node_index = findfirst(i->begin
													node = node_map[WaypointID(segment.id, lane.id.lane, ids[i])]
				...
				nodes = map(i->node_map[WaypointID(segment.id, lane.id.lane, ids[i])], [first_node_index:final_node_index])

				streetsegment = get_segment!(tile, segment.id)

				if has_lane(streetsegment, lane.id.lane)
					# need to stitch lanes together
					streetlane = streetsegment.lanes[streetlane.id]
	...
	for tile in values(sn.tile_dict)
		for seg in values(tile.segments)
			for lane in values(seg.lanes)
				...
				count = 1
				if has_prev
					pts[1,count] = node_prev.pos.x
					pts[2,count] = node_prev.pos.y
					count += 1
				end
				for node in nodes
					pts[1,count] = node.pos.x
					pts[2,count] = node.pos.y
					count += 1
				end
				if has_next
					pts[1,count] = node_next.pos.x
					pts[2,count] = node_next.pos.y
				end
				...
				if count > 1
					...
					if has_prev
						...
						lane.curve = Curve(lane.id, c.s - pt[SIND], c.x, c.y, c.t, c.k, c.k_d)
					end
					lane.has_leading_node = has_prev
					lane.has_training_node = has_next
				else
		...
		for seg in values(tile.segments)
			for lane in values(seg.lanes)
				...
				for node in lane.nodes
					...
					node.extind = extind
					node.d_along = curve_at(lane.curve, extind)[SIND]
...
function _calc_num_lanes_on_side!(sn::StreetNetwork, center_tile::NetworkTile, node::StreetNode)
	...
	θ   = curve_at(get_lane(center_tile, node.id).curve, node.extind)[TIND]
	# unit_v = [cos(θ), sin(θ)]
	p_orig = [x,y]
		...
		for dist in DIST_DISCRETIZATION
			...
			if proj.successful
				...
				if !in(lanetag, lanes_seen) # found a new one!
					perp_dist = hypot(x - proj.curvept[XIND], y - proj.curvept[YIND])
					if LANE_SEP_MIN < perp_dist < LANE_SEP_MAX
						node.n_lanes_left += 1
...
function curves2streetmap(curves::CurveSet; verbosity::Int=0)
	rndf = curves2rndf(curves)
	rndf2streetmap(rndf, verbosity=verbosity, convert_ll2utm = false)
end
function curves2rndf(curves::CurveSet)

	const NODE_SEPARATION = 5.0
