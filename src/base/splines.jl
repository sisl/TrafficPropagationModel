# NOTE(tim): in the future we should use new package Dierckx for splines
...
function sample_spline(spline_coeffs::AbstractMatrix{Float64}, t::Float64)
	...
	col_ind = clamp(ceil(t), 1, size(spline_coeffs,2))
...
function arclength(
	...
	len = 0.0
	for i = int(floor(t_min)) : min(int(floor(t_max)), n-1)
...
function plot_spline(spline_coeffs_x, spline_coeffs_y, t_arr::Vector{Float64})
	figure()
	plot(sample_spline(spline_coeffs_x, t_arr), sample_spline(spline_coeffs_y, t_arr))
	grid(true)

	L = calc_curve_length(spline_coeffs_x, spline_coeffs_y)
	s_arr = linspace(0.0, L,300)
	t2_arr = map(s->calc_curve_param_given_arclen(spline_coeffs_x, spline_coeffs_y, s, curve_length=L), s_arr)

	figure()
	plot(s_arr, rad2deg(sample_spline_theta(spline_coeffs_x, spline_coeffs_y, t2_arr)), color=[1.0,0.0,0.0])
	plot(s_arr, rad2deg(sample_spline_curvature(spline_coeffs_x, spline_coeffs_y, t2_arr)), color=[0.0,1.0,0.0])
	plot(s_arr, rad2deg(sample_spline_derivative_of_curvature(spline_coeffs_x, spline_coeffs_y, t2_arr)), color=[0.0,0.0,1.0])
	grid(true)
	legend(["theta", "curvature", "k_d"])
end
