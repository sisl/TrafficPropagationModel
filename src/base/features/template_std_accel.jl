...
for ticks in tick_list
	...
	lstr = latexstring("\\sigma^{a^F_x}_{$counts$unit}")

	create_feature_basics( fname_str, UnextractableFeature, "m/s2", false, false, Inf, 0.0, true, sym_feature, lstr, str_description)
	...
	@eval begin
		function _get(::$feature_name, pdset::PrimaryDataset, carind::Int, validfind::Int)
			...
			for jump = -1:-1:maxjump
				...
				push!(vals, (fut - cur)/SEC_PER_FRAME)
			end
	...
	lstr = latexstring("\\sigma^{a^F_y}_{$counts$unit}")
